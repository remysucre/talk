...
\begin{document}
  ...
  \begin{frame}{Automated Reasoning for Data Processing}
    \textbf{Chase \& Backchase CITE}: Given $Q$, find the smallest~\footnote{
  ...
  \begin{frame}{The Functional Representation of Data}
    ...
    where $\SR$ is a commutative semiring CITE. \pause
    \begin{itemize}
  ...
  \begin{frame}{The Functional Representation of Data}
    \alert{Isomorphism Captures Equivalence CITE} ($\bigstar$)
  ...
  \begin{frame}{Searching the Equivalence Space}
    ...
    An \textbf{E-Graph CITE} is a compact data structure to store collections 
  ...
  \begin{frame}{Searching the Equivalence Space}
    ...
    one per equivalence class ($B_{c}$), then solve CITE :
    \begin{align*}
  ...
  \begin{frame}{Searching the Efficient Space}
    ...
    \textbf{Solution CITE} Find a function $H$ such that:
  ...
  \begin{frame}{Searching the Efficient Space}
    How to find $H$? Counterexample-guided Synthesis CITE (CEGIS):
    \begin{figure}
      \includegraphics[width=0.7\linewidth]{cegis.pdf}
    \end{figure}
  ...
  \begin{frame}{Searching the Efficient Space}
    ...
    \textbf{Solution CITE} Canonize input query $G \circ F$ before CEGIS,
 ...
 \[\forall f : \bigwedge_{i \in [k]} x_i = y_i \implies
  ...
  \begin{frame}{Congruence in Cascade-style Optimizers}
    ...
    CockroachDB CITE (distributed SQL database):
    \begin{figure}
      \includegraphics[width=0.6\linewidth]{cockroach.jpeg}
    \end{figure}
    Green Plum Orca CITE (Modular optimizer):
    \begin{figure}
